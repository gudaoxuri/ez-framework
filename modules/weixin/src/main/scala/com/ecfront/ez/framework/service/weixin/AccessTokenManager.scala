...
object AccessTokenManager extends BaseProcessor {
  ...
  private def reFetchAccessToken(): (String, Long) = {
    val result = getData(s"token?grant_type=client_credential&appid=${ServiceAdapter.appId}&secret=${ServiceAdapter.secret}")
    (result("access_token").asInstanceOf[String], result("expires_in").asInstanceOf[Long])
  }
