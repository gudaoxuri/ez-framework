...
import com.ecfront.ez.framework.service.auth.test.AuthStartupSpec
...
class MessageSpec extends AuthStartupSpec {
  ...
  val baseURL = "http://127.0.0.1:8080/message"
  ...
  test("Message Test") {
    ...
    // 个人消息
    MessageService.sendToAccount("@sysadmin", "cate1", "0", "msgSysAdmin1", "", startTime, endTime)
    // 个人消息（使用模板）
    ...
    // 个人消息，不在时间点
    MessageService.sendToAccount("@sysadmin", "cate1", "0", "msgSysAdmin3", "", endTime, endTime)
    MessageService.sendToRole("@admin", "cate1", "0", "msgAdmin", "", startTime, endTime)

    val token =
      RespHttpClientProcessor.post[OptInfo]("http://127.0.0.1:8080/public/auth/login/", Map("id" -> "sysadmin", "password" -> "admin")).body.token
    assert(
