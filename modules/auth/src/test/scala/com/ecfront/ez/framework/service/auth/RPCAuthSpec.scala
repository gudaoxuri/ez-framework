...
class RPCAuthSpec extends MockStartupSpec {
  ...
  test("RPC auth test") {
    ...
    var loginInfoResp = JsonHelper.toObject[Resp[Token_Info_VO]](
      HttpClientProcessor.post("http://127.0.0.1:8080/public/auth/login/", Map("login_id" -> "admin", "password" -> "admin")))
    assert(!loginInfoResp)
    loginInfoResp = JsonHelper.toObject[Resp[Token_Info_VO]](
      HttpClientProcessor.post("http://127.0.0.1:8080/public/auth/login/", Map("login_id" -> "sysadmin", "password" -> "admin")))
    assert(loginInfoResp && loginInfoResp.body.login_name == "Sys Admin")
