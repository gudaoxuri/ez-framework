...
class AuthMockSpec extends FunSuite {
  ...
  test("auth 测试") {
    ...
    assert(token != "")
    println("Token:" + token)
    val loginInfo = JsonHelper.toGenericObject[Resp[Token_Info_VO]](HttpHelper.get(s"http://127.0.0.1:8080/auth/logininfo/?_token_=$token").body).body
    ...
    JsonHelper.toGenericObject[Resp[String]](HttpHelper.post(s"http://127.0.0.1:8080/auth/manage/account/?_token_=$token",account).body)
    val getAccount = JsonHelper.toGenericObject[Resp[EZ_Account]](HttpHelper.get(s"http://127.0.0.1:8080/auth/manage/account/testUser/?_token_=$token").body).body
    ...
    JsonHelper.toGenericObject[Resp[Void]](HttpHelper.get(s"http://127.0.0.1:8080/auth/logout/?_token_=$token").body)
    val getAccountWrap = JsonHelper.toGenericObject[Resp[EZ_Account]](HttpHelper.get(s"http://127.0.0.1:8080/auth/manage/account/testUser/?_token_=$token").body)
